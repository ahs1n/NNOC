...
class FamilyMembersListActivity : AppCompatActivity() {
                        ...
                        when (item.id) {
                            0 -> {
                                startActivityForResult(
                                    Intent(
                                        this,
                                        SectionA2Activity::class.java
                                    ).putExtra(SERIAL_EXTRA, serial), CONSTANTS.MEMBER_ITEM
                                )
                            }
                            1 -> {
                                ...
                                indexKishMWRA = mainVModel.mwraChildU5to10Lst.value?.get(
                                    kishSelectedMWRA(
                                        ...
                                        mainVModel.mwraChildU5to10Lst.value!!.size
                                    ) - 1
                                )

                                if (indexKishMWRA != null) {
                                    val childLst =
                                        mainVModel.getAllChildrenOfSelMWRA(indexKishMWRA.serialno.toInt())
                                    indexKishMWRAChild = childLst?.let {
                                        childLst[kishSelectedMWRA(
                                            intent.getIntExtra("sno", 0),
                                            childLst.size
                                        ) - 1]
                                    }
                                    ...
                                    GlobalScope.launch {
                                        val indexMwraUpdate =
                                            async { updateKishMember(indexKishMWRA, 1) }
                                        val indexChildUpdate =
                                            async { updateKishMember(indexKishMWRAChild, 2) }
                                        if (indexMwraUpdate.await()
                                                .let { true } and indexChildUpdate.await()
                                                .let { true }
                                        ) {
                                            finish()
                                            startActivity(
                                                Intent(
                                                    this@FamilyMembersListActivity,
                                                    SectionA31Activity::class.java
                                                )
                                            )
                                        }
                                    }
    ...
    private fun settingValue() {
        ...
        mainVModel.childLstU5to10.observe(
            this,
            Observer { item ->
                bi.contentScroll.under5To10.text = String.format("%02d", item.size)
            })
        mainVModel.mwraLst.observe(
            this,
            Observer { item -> bi.contentScroll.mwra.text = String.format("%02d", item.size) })
    ...
    private fun setupRecyclerView(membersLst: MutableList<FamilyMembersContract>) {
        ...
        adapter.setItemClicked { item, position ->
            ...
            setItemClick {
                ...
                startActivityForResult(
                    Intent(this, SectionA2Activity::class.java)
